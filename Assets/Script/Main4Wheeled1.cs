...
public class Main4Wheeled : MonoBehaviour
{
    ...
    public GameObject backLeftWheelGameObject;  // Reference to the back left wheel game object.
    public GameObject backRightWheelGameObject;  // Reference to the back right wheel game object.
    public GameObject frontLeftWheelGameObject;  // Reference to the front left wheel game object.
    public GameObject frontRightWheelGameObject;  // Reference to the front right wheel game object.
    public GameObject frontLeftWheelVisual;  // Visual representation of the front left wheel.
    public GameObject frontRightWheelVisual;  // Visual representation of the front right wheel.
    public GameObject backRightWheelVisual;  // Visual representation of the back right wheel.
    public GameObject backLeftWheelVisual;  // Visual representation of the back left wheel.

    private WheelCollider backLeftWheel;  // Wheel collider for the left wheel.
    private WheelCollider backRightWheel;  // Wheel collider for the right wheel.
    private WheelCollider frontLeftWheel;  // Wheel collider for the front wheel.
    private WheelCollider frontRightWheel;  // Wheel collider for the front wheel.
    ...
    private float interaxialDistance;  // Distance between the two front wheels.
    ...
    private List<float> steeringAngles;
    ...
    void Start()
    {
        ...
        backLeftWheel = backLeftWheelGameObject.GetComponent<WheelCollider>();
        backRightWheel = backRightWheelGameObject.GetComponent<WheelCollider>();
        frontLeftWheel = frontLeftWheelGameObject.GetComponent<WheelCollider>();
        frontRightWheel = frontRightWheelGameObject.GetComponent<WheelCollider>();

        // Calculate vehicle dimensions
        r = frontLeftWheel.radius;
        var z1 = backLeftWheelGameObject.transform.position.z;
        var z2 = frontLeftWheelGameObject.transform.position.z;
        var x1 = frontLeftWheelGameObject.transform.position.x;
        var x2 = frontRightWheelGameObject.transform.position.x;

        L = Mathf.Abs(z1 - z2);
        interaxialDistance = Mathf.Abs(x1 - x2);
    ...
    void Update()
    {
        ...
        if (path != null)
        {
            ...
            if (final_target_distance <= TOL)
            {
                ...
                backLeftWheel.brakeTorque = 10;
                backRightWheel.brakeTorque = 10;
                return;
            }

            // Calculate desired angular velocity (omega_ref)
            omega_ref = vlin / r;
            omega_l = backLeftWheel.rpm * 2 * Mathf.PI / 60;

            var e_left = omega_ref - omega_l;
            var pid_update = PIDController.UpdatePID(e_left, dt);

            // Apply motor torque and steering angle
            backLeftWheel.motorTorque = pid_update;
            backRightWheel.motorTorque = pid_update;
            steeringAngles = ppc.ComputeSteeringAngle(path, transform, L, interaxialDistance);

            frontLeftWheel.steerAngle = steeringAngles[0];  // Steering angle calculated by PPC.
            frontRightWheel.steerAngle = steeringAngles[1];  // Steering angle calculated by PPC.

            // Update wheel visuals
            utils.UpdateWheelVisual(frontLeftWheel, frontLeftWheelVisual);
            utils.UpdateWheelVisual(frontRightWheel, frontRightWheelVisual);
            utils.UpdateWheelVisual(backRightWheel, backRightWheelVisual);
            utils.UpdateWheelVisual(backLeftWheel, backLeftWheelVisual);
            ...
            vel = rb.velocity.z;
            writer.WriteLine(timer + " " + omega_ref + " " + backLeftWheel.rpm * 2 * Mathf.PI / 60 + " " + frontLeftWheel.steerAngle + " " + vel);
        }
    }

    // Compute the path to a target position using AStar
